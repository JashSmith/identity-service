...
public sealed class ArchitectureTests
{
    ...
    {
        var names = typeof(Identity.Domain.PermissionDefinition).Assembly.GetTypes().Where(x => x.Namespace == "Identity.Domain").Select(x => x.Name).ToArray();
    ...
    public void Contracts_have_no_private_key_material()
    {
        ...
        foreach (var tp in types) foreach (var pr in tp.GetProperties())
    ...
    {
        var names = typeof(Identity.Application.KeyRotationService).Assembly.GetReferencedAssemblies().Select(x=>x.Name).ToArray();
