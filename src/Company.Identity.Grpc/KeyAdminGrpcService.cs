...
public sealed class KeyAdminGrpcService(KeyRotationService svc) : KeyAdminService.KeyAdminServiceBase
{
    ...
    private static RotationOperation ToOpProto(RotationOperationDto o) => new()
    {
        ...
        CurrentStep = o.CurrentStep, Status = o.Status, StartedAt = o.StartedAt.ToString("O"), UpdatedAt = o.UpdatedAt.ToString("O"),
    ...
    {
        var dto = await svc.GenerateAsync(new global::Identity.Contracts.GenerateKeyRequest(request.Size == 0 ? 2048 : request.Size, string.IsNullOrWhiteSpace(request.Kid) ? null : request.Kid, request.Reason), Actor(ctx), ctx.CancellationToken);
        return ToProto(dto);
    }

    public override async Task<SigningKey> ImportKey(ImportKeyRequest request, ServerCallContext ctx)
    {
        var dto = await svc.ImportAsync(new global::Identity.Contracts.ImportKeyRequest(request.Pem, string.IsNullOrWhiteSpace(request.Kid) ? null : request.Kid, request.Reason), Actor(ctx), ctx.CancellationToken);
        return ToProto(dto);
    }

    public override async Task<SigningKeyDetail> GetKey(GetKeyRequest request, ServerCallContext ctx)
    {
        var d = await svc.GetAsync(request.Kid, ctx.CancellationToken) ?? throw new RpcException(new Status(StatusCode.NotFound, $"key {request.Kid} not found"));
        return new SigningKeyDetail { Key = ToProto(d.Key), PresentInJwks = d.PresentInJwks, JwksFingerprintMatch = d.JwksFingerprintMatch ?? "" };
    }

    public override async Task<ListKeysResponse> ListKeys(ListKeysRequest request, ServerCallContext ctx)
    {
        var l = await svc.ListAsync(request.Page <= 0 ? 1 : request.Page, request.PageSize <= 0 ? 20 : request.PageSize, ctx.CancellationToken);
    ...
    {
        var dto = await svc.RotateAsync(new global::Identity.Contracts.RotateRequest(request.Reason, string.IsNullOrWhiteSpace(request.IdempotencyKey) ? null : request.IdempotencyKey, request.Size == 0 ? null : request.Size, string.IsNullOrWhiteSpace(request.TargetKid) ? null : request.TargetKid), Actor(ctx), ctx.CancellationToken);
        return ToOpProto(dto);
    }

    public override async Task<RotationOperation> Rollback(RollbackRequest request, ServerCallContext ctx)
    {
        var dto = await svc.RollbackAsync(request.Kid, new global::Identity.Contracts.RollbackRequest(request.Reason, string.IsNullOrWhiteSpace(request.IdempotencyKey) ? null : request.IdempotencyKey), Actor(ctx), ctx.CancellationToken);
        return ToOpProto(dto);
    }
    ...
    public override async Task<RotationState> GetRotationState(GetRotationStateRequest request, ServerCallContext ctx)
    {
        ...
        r.PassiveKids.AddRange(s.PassiveKids); r.DisabledKids.AddRange(s.DisabledKids);
