    ...
    IAuditSink audit)
{
    private string NewKid()
        => string.Concat(options.KidPrefix, DateTimeOffset.UtcNow.ToString("yyyy-MM"), "-", Guid.NewGuid().ToString("N").AsSpan(0, 8));
        ...
        Identity.Contracts.GenerateKeyRequest request, string actor, CancellationToken ct)
    {
        var size = request.Size is 2048 or 3072 or 4096
            ? (RsaKeySize)request.Size : options.DefaultSize;
        ...
        using (priv)
        {
            var vaultRef = await vault.StorePrivateKeyAsync(kid, size, priv.Pem, publicPem, fingerprint, options.Realm, ct);
            ...
            await audit.RecordAsync(new AuditEvent(actor, "key.generated", kid, now,
                new Dictionary<string, string?> { ["kid"] = kid, ["size"] = ((int)size).ToString(), ["vault_version"] = vaultRef.Version.ToString() }), ct);
            return ToDto(meta);
        }
    }
        ...
        Identity.Contracts.ImportKeyRequest request, string actor, CancellationToken ct)
    {
        ...
        using (priv)
        {
            var vaultRef = await vault.StorePrivateKeyAsync(requestedKid, size, priv.Pem, publicPem, fingerprint, options.Realm, ct);
            var now = clock.GetUtcNow();
    ...
    public async Task<Identity.Contracts.SigningKeyDto> StageAsync(string kid, string actor, CancellationToken ct)
    {
        ...
        using (privatePem)
        {
            var component = await keycloak.RegisterPassiveAsync(options.Realm, kid, meta.Size, privatePem.Pem, publicPem, ct);
            var now = clock.GetUtcNow();
            var next = meta with
            {
            ...
            await repo.AddHistoryAsync(options.Realm, kid, meta.State.ToString(), next.State.ToString(), now, actor, null, ct);
    ...
    public async Task<Identity.Contracts.SigningKeyDto> ValidateAsync(string kid, string actor, CancellationToken ct)
    {
        ...
        if (!result.Present || !result.FingerprintMatches)
        {
            ...
            await repo.AddHistoryAsync(options.Realm, kid, meta.State.ToString(), failed.State.ToString(), now, actor, result.Reason, ct);
    ...
    public async Task<Identity.Contracts.SigningKeyDto> ActivateAsync(string kid, string actor, string? idempotencyKey, CancellationToken ct)
    {
        ...
        var handle = await rotationLock.TryAcquireAsync(options.Realm, options.RotationLockTtl, ct)
            ?? throw new InvalidOperationException($"Rotation lock for realm '{options.Realm}' is held by another operation.");
        try
        {
            ...
            await repo.AddHistoryAsync(options.Realm, kid, current.State.ToString(), activated.State.ToString(), now, actor, null, ct);
            ...
            if (previousActive is not null)
            {
                ...
                await repo.AddHistoryAsync(options.Realm, previousActive.Kid, previousActive.State.ToString(), passivated.State.ToString(), now, actor, $"superseded by {kid}", ct);
        ...
        finally { await rotationLock.ReleaseAsync(handle, ct); }
    }
    ...
    private async Task PollConvergenceAsync(string newKid, string? oldKid, CancellationToken ct)
    {
        ...
        throw new TimeoutException($"JWKS convergence timed out waiting for kid '{newKid}'" + (oldKid is null ? "" : $" and retention of '{oldKid}'"));
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(request.Reason)) throw new ArgumentException("Reason is required for rotation.", nameof(request));
        var idem = string.IsNullOrWhiteSpace(request.IdempotencyKey) ? Guid.NewGuid().ToString("N") : request.IdempotencyKey.Trim();
        ...
        if (string.IsNullOrEmpty(targetKid))
        {
            ...
            var gen = await GenerateAsync(new Identity.Contracts.GenerateKeyRequest((int)size, Reason: request.Reason), actor, ct);
            targetKid = gen.Kid;
        }
        else
        {
            ...
            if (existing.State != KeyLifecycleState.VaultStored && existing.State != KeyLifecycleState.Passive && existing.State != KeyLifecycleState.Validated)
                throw new InvalidOperationException($"Target '{targetKid}' is {existing.State}, expected VaultStored/Passive/Validated.");
        }

        var opId = NewOpId();
        var now = clock.GetUtcNow();
        var op = new KeyRotationOperation(opId, idem, options.Realm, targetKid, null, KeyLifecycleState.VaultStored, SagaStatus.InProgress, now, now, actor);
        await repo.UpsertOperationAsync(op, ct);
    ...
    public Task<Identity.Contracts.RotationOperationDto> EmergencyRotateAsync(
        ...
        => RotateAsync(new Identity.Contracts.RotateRequest(request.Reason, request.IdempotencyKey, request.Size), actor, ct);
    ...
    {
        if (string.IsNullOrWhiteSpace(request.Reason)) throw new ArgumentException("Reason is required for rollback.", nameof(request));
        ...
        var all = await repo.ListAsync(options.Realm, 1, 200, ct);
        var previous = all.FirstOrDefault(x => x.State == KeyLifecycleState.InGrace || x.State == KeyLifecycleState.Retiring)
        ...
        try
        {
            ...
            await repo.AddHistoryAsync(options.Realm, previous.Kid, previous.State.ToString(), reactivated.State.ToString(), now, actor, $"rollback from {failedKid}: {request.Reason}", ct);
            await repo.AddHistoryAsync(options.Realm, failedKid, failed.State.ToString(), demoted.State.ToString(), now, actor, $"rolled back: {request.Reason}", ct);
            var op = new KeyRotationOperation(NewOpId(), request.IdempotencyKey ?? NewOpId(), options.Realm, failedKid, previous.Kid,
    ...
    {
        var meta = await repo.GetAsync(options.Realm, kid, ct) ?? throw new KeyNotFoundException($"Key '{kid}' not found.");
    ...
    public async Task DestroyAsync(string kid, Identity.Contracts.DestroyRequest request, string actor, CancellationToken ct)
    {
        if (request.ConfirmKid != kid) throw new ArgumentException("ConfirmKid must equal the target kid.", nameof(request));
        var meta = await repo.GetAsync(options.Realm, kid, ct) ?? throw new KeyNotFoundException($"Key '{kid}' not found.");
        var assessment = await safety.AssessAsync(options.Realm, kid, ct);
        if (!assessment.Safe)
            throw new InvalidOperationException($"Destroy blocked for '{kid}': {string.Join("; ", assessment.BlockingReasons)}");
        var now = clock.GetUtcNow();
        ...
        await repo.UpsertAsync(next, ct);
        await repo.AddHistoryAsync(options.Realm, kid, meta.State.ToString(), next.State.ToString(), now, actor, request.Reason, ct);
    ...
    public Task<IReadOnlyCollection<Identity.Contracts.KeyHistoryEntryDto>> HistoryAsync(string kid, CancellationToken ct)
    ...
    public async Task<Identity.Contracts.RotationStateDto> RotationStateAsync(CancellationToken ct)
    {
        ...
        var passive = all.Where(x => x.State is KeyLifecycleState.Passive or KeyLifecycleState.Validated or KeyLifecycleState.InGrace).Select(x => x.Kid).ToArray();
        var disabled = all.Where(x => x.State is KeyLifecycleState.Disabled or KeyLifecycleState.Retired).Select(x => x.Kid).ToArray();
    ...
    private static Identity.Contracts.RotationOperationDto ToOpDto(KeyRotationOperation op) =>
        new(op.OperationId, op.Realm, op.TargetKid, op.PreviousKid, op.CurrentStep.ToString(), op.Status.ToString(), op.StartedAt, op.UpdatedAt, op.Actor, op.FailureReason);
}
