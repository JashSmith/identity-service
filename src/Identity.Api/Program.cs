...
if (!string.IsNullOrWhiteSpace(oidcAuthority))
{
    ...
    {
        client.Timeout =
            TimeSpan.FromSeconds(builder.Configuration.GetValue("Identity:ExternalProviders:Oidc:TimeoutSeconds",
                10));
    });
    var oidcOptions = new OpenIdConnectOptions
    {
        ...
        Timeout = TimeSpan.FromSeconds(builder.Configuration.GetValue("Identity:ExternalProviders:Oidc:TimeoutSeconds",
            10))
    };
    ValidateExternalProviderOptions(oidcOptions.Authority, oidcOptions.ClientId, oidcOptions.RequireHttpsMetadata,
        oidcOptions.Timeout, "OIDC");
    builder.Services.AddSingleton(oidcOptions);
    builder.Services.AddSingleton<IExternalIdentityProvider>(sp =>
        sp.GetRequiredService<OpenIdConnectIdentityProvider>());
}
...
if (!string.IsNullOrWhiteSpace(keycloakAuthority))
{
    builder.Services.AddHttpClient<KeycloakIdentityProvider>(client =>
    {
        client.Timeout =
            TimeSpan.FromSeconds(
                builder.Configuration.GetValue("Identity:ExternalProviders:Keycloak:TimeoutSeconds", 10));
    });
    ...
    ValidateExternalProviderOptions(keycloakOptions.Authority, keycloakOptions.ClientId,
        keycloakOptions.RequireHttpsMetadata, keycloakOptions.Timeout, "Keycloak");
...
builder.Services.AddSingleton<Identity.Application.ISystemClock>(_ =>
    new Identity.Application.SystemClock(TimeProvider.System));
...
builder.Services.AddOptions<PermissionManifestOptions>()
    ...
    .Validate(x => !string.IsNullOrWhiteSpace(x.ServiceId) && !string.IsNullOrWhiteSpace(x.ServiceName),
        "Permission manifest service metadata is required.")
    .ValidateOnStart();
...
using (var scope = app.Services.CreateScope())
{
        ...
        !db.Users.Any(x => x.Username == username))
    {
        ...
        db.Users.Add(user);
        db.PasswordCredentials.Add(new PasswordCredential(user.Id,
            scope.ServiceProvider.GetRequiredService<IPasswordVerifier>().Hash(password), clock.UtcNow));
        db.SaveChanges();
    }
}
...
app.MapPost("/api/auth/login",
    async (LoginRequest request, LocalAuthenticationService authentication, CancellationToken cancellationToken) =>
    {
        var result = await authentication.LoginAsync(request.Username, request.Password, cancellationToken);
        if (!result.Succeeded) return Results.Unauthorized();
        return Results.Ok(new TokenResponse(result.AccessToken!.AccessToken, result.RefreshToken!,
            result.AccessToken.ExpiresAt, result.SessionId!.Value));
    });
app.MapPost("/api/auth/refresh",
    async (RefreshRequest request, LocalAuthenticationService authentication, CancellationToken cancellationToken) =>
    {
...
app.MapGet("/api/auth/session",
    (HttpContext context) => Results.Ok(new { authenticated = context.User.Identity?.IsAuthenticated == true }));
    ...
    CancellationToken cancellationToken) =>
{
    ...
    return result.Succeeded
        ? Results.NoContent()
        : Results.Conflict(new ProblemResponse(
    ...
    CancellationToken cancellationToken) =>
{
    ...
    await context.SignInAsync(ServiceCollectionExtensions.BffScheme,
    ...
    string providerName)
{
    if (!Uri.TryCreate(authority, UriKind.Absolute, out var authorityUri) ||
        ...
        throw new InvalidOperationException(
            $"{providerName} authority must be an absolute HTTPS URI when HTTPS metadata is required.");
