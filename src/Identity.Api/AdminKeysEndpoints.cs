...
public static class AdminKeysEndpoints
{
    public static RouteGroupBuilder MapAdminKeys(this IEndpointRouteBuilder app)
    {
        ...
        g.MapPost("/generate",
            async (GenerateKeyRequest req, KeyRotationService svc, HttpContext ctx, CancellationToken ct) =>
            {
        ...
        g.MapPost("/import",
            async (ImportKeyRequest req, KeyRotationService svc, HttpContext ctx, CancellationToken ct) =>
            {
                var actor = ctx.User.Identity?.Name ?? "admin";
        ...
        g.MapPost("/{kid}/stage", async (string kid, KeyRotationService svc, HttpContext ctx, CancellationToken ct) =>
                Results.Ok(await svc.StageAsync(kid, ctx.User.Identity?.Name ?? "admin", ct)))
            .RequireAuthorization(KeyPermissions.Stage);
        g.MapPost("/{kid}/validate",
                async (string kid, KeyRotationService svc, HttpContext ctx, CancellationToken ct) =>
                    Results.Ok(await svc.ValidateAsync(kid, ctx.User.Identity?.Name ?? "admin", ct)))
            .RequireAuthorization(KeyPermissions.Validate);
        g.MapPost("/{kid}/activate", async (string kid, string? idempotencyKey, KeyRotationService svc, HttpContext ctx,
                    CancellationToken ct) =>
                Results.Ok(await svc.ActivateAsync(kid, ctx.User.Identity?.Name ?? "admin", idempotencyKey, ct)))
            .RequireAuthorization(KeyPermissions.Activate);
        g.MapPost("/{kid}/retire", async (string kid, KeyRotationService svc, HttpContext ctx, CancellationToken ct) =>
                Results.Ok(await svc.RetireAsync(kid, ctx.User.Identity?.Name ?? "admin", ct)))
            .RequireAuthorization(KeyPermissions.Retire);
        g.MapPost("/{kid}/destroy",
            async (string kid, DestroyRequest req, KeyRotationService svc, HttpContext ctx, CancellationToken ct) =>
            {
        ...
        g.MapPost("/rotate", async (RotateRequest req, KeyRotationService svc, HttpContext ctx, CancellationToken ct) =>
                Results.Ok(await svc.RotateAsync(req, ctx.User.Identity?.Name ?? "admin", ct)))
            .RequireAuthorization(KeyPermissions.Rotate);
        g.MapPost("/rotate/emergency",
                async (EmergencyRotateRequest req, KeyRotationService svc, HttpContext ctx, CancellationToken ct) =>
                    Results.Ok(await svc.EmergencyRotateAsync(req, ctx.User.Identity?.Name ?? "admin", ct)))
            .RequireAuthorization(KeyPermissions.EmergencyRotate);
        g.MapPost("/{kid}/rollback",
                async (string kid, RollbackRequest req, KeyRotationService svc, HttpContext ctx,
                        CancellationToken ct) =>
                    Results.Ok(await svc.RollbackAsync(kid, req, ctx.User.Identity?.Name ?? "admin", ct)))
            .RequireAuthorization(KeyPermissions.Rollback);
