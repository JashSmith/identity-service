...
) : IKeyRetirementSafety
{
    public async Task<RetirementSafetyAssessment> AssessAsync(string realm, string kid, CancellationToken ct)
    {
        ...
        if (m.State is KeyLifecycleState.Generated or KeyLifecycleState.VaultStored or KeyLifecycleState.Passive
            or KeyLifecycleState.Validated) r.Add($"key in state {m.State} not eligible for destroy");
