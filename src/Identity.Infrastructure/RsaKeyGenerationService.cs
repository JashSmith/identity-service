...
{
    public (string PublicPem, Identity.Application.TransientPrivatePem Private, string PublicFingerprint) Generate(RsaKeySize size, string kid)
    {
    ...
    public (string PublicPem, Identity.Application.TransientPrivatePem Private, RsaKeySize Size, string PublicFingerprint) ParseImport(string privatePem, string kid)
    {
        using var rsa = RSA.Create();
        rsa.ImportFromPem(privatePem);
        var size = rsa.KeySize switch { 2048 => RsaKeySize.Rsa2048, 3072 => RsaKeySize.Rsa3072, 4096 => RsaKeySize.Rsa4096, _ => throw new ArgumentException($"Unsupported RSA size {rsa.KeySize}.", nameof(privatePem)) };
        var publicPem = ExportPublicPem(rsa);
